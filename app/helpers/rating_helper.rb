module RatingHelper
  def rating_for(rateable_obj, dimension=nil, options={})
    ...
    current_rate_obj = Rate.where(rater_id: current_user.id, rateable_id: rateable_obj.id)
    current_star = current_rate_obj[0] ? current_rate_obj[0].stars : 0
    ...
    if options[:imdb_avg] && readonly
      ...
      content_tag :div, '', "data-dimension" => dimension, :class => "star", "data-rating" => current_star,
